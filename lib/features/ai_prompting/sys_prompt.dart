...
- toggle_habit_completion(habitName, isCompleted, timeBoxId?, date?): Toggles a habit's completion status in the schedule for the specified date (defaults to today)
...
<function>{ "description": "Toggles the completion status of a habit in the schedule screen. This function allows checking or unchecking habits for the current day, updating their status across all timeboxes or in a specific timebox. When a habit is marked as completed, points are awarded; when unchecked, points are removed.", "name": "toggle_habit_completion", "parameters": { "properties": { "habitName": { "description": "The name of the habit to toggle. The function will search for habits with similar names.", "type": "string" }, "isCompleted": { "description": "The new completion status for the habit (true = completed/checked, false = not completed/unchecked).", "type": "boolean" }, "timeBoxId": { "description": "Optional. The specific timebox ID to update the habit in. If not provided, the habit status will be updated across all timeboxes for today.", "type": "integer" }, "date": { "description": "Optional. The date for which to toggle the habit completion status. If not provided, the habit status will be toggled for today.", "type": "string" } }, "required": [ "habitName", "isCompleted" ] } }</function>
</functions>
